...
test_that("Subsetting data on Summarized Experiment while dropping rest of summarized experiments", {
  data("mae")
  mae1=transmute(mae,subset = cyto[1:5, 1:3])
  expect_equal(dim(mae1[[1]]),dim(mae[[2]][1:5, 1:3]) )
  expect_equal(mae1[['subset']], mae[[2]][1:5, 1:3])
  expect_s4_class(mae1[[1]], "SummarizedExperiment")
})

test_that("Adding new assay and drops existing assays from Summarized Experiment", {
  data("mae")
  mae3=mae %>% transmute(cyto, plus_one = cyto_conc + 1)
  expect_equal(c(assayNames(mae3[[2]])), c("plus_one"))
})