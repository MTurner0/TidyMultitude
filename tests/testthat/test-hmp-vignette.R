...
momspi_data <- MultiAssayExperiment(
  experiments = list(phy16S = momspi16S,
                     cyto = momspiCyto)
  )
...
combined_samp <- merge(
  ...
  momspiCyto_samp,
  by = c(
    "subject_id", "sample_body_site", "project_name", "study_full_name",
    "subject_gender", "subject_race", "visit_number"
    )
  )
...
sample_data(combined_16S_phyloseq) <- momspi16S_samp[
  momspi16S_samp$file_name %in% combined_samp$file_name.x,
  ]

# REPLACE combined_16S_phyloseq <- subset_samples(momspi16S_phyloseq, file_name
# %in% combined_samp$file_name.x)
...
target1_cyto <- momspiCyto_mtx[
  , colnames(momspiCyto_mtx) %in% combined_samp$file_name.y
  ]
...
test_that("HMP Vignette: checkpoint 1", {
  expect_equal(
    apply(checkpoint1_phy16S, 1, mean), apply(target1_phy16S, 1, mean)
    )
  expect_equal(
    apply(checkpoint1_cyto, 1, mean), apply(target1_cyto, 1, mean)
    )
})
...
test_that("HMP Vignette: checkpoint 2", {
  expect_equal(
    apply(checkpoint2_phy16S, 1, mean), apply(target2_phy16S, 1, mean)
    )
  expect_equal(
    apply(checkpoint2_cyto, 1, mean), apply(target2_cyto, 1, mean)
    )
})

## PASSES CHECKPOINT 2

target3_phy16S <- ade4::dudi.pca(
  taxa_mtx, scannf = FALSE, nf = 61, center = FALSE, scale = FALSE
  )
target3_cyto <- ade4::dudi.pca(
...
target_coin <- ade4::coinertia(
  target3_phy16S, target3_cyto, scannf = FALSE, nf = 2
  )

checkpoint_coin <- ade4::coinertia(
  checkpoint3_phy16S, checkpoint3_cyto, scannf = FALSE, nf = 2
  )
